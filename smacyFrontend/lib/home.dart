...
class _HomeState extends State<Home> {
  ...
  Future<void> getBaseUrl() async {
    // 2
    final response = await http.get(Uri.parse("http://10.0.2.2:8000/homeMovie/"));
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        return GestureDetector(
                          onTap: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(builder: (context) => DownloadPage(movieId: movieList[index].keys.first, userId: widget.userId)),
                            );
                          },
                          child: Poster(
